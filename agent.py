...
class KnowledgeAwareAgent:
    ...
    def __init__(self, graph, opt, tokenizer=None, rel_extractor = None, device=None, finetune=False) -> None:
        ...
        self.bigbird_tokenizer = BigBirdTokenizer.from_pretrained('google/bigbird-roberta-base')
    ...
    def _lf_tokenize_obs(self, texts):
        return self.bigbird_tokenizer(texts, return_tensors="pt", padding=True, truncation=True).to(self.device)

    def _lf_tokenize_cmds(self, batch_size, infos):
        command_list = []
        for b in range(batch_size):
            # TODO: Verify correct sizing
            cmd_b = self.bigbird_tokenizer(infos["admissible_commands"][b],return_tensors="pt",padding=True, truncation=True)
